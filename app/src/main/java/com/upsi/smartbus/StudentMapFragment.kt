...
class StudentMapFragment : Fragment(), OnMapReadyCallback {
    ...
    private fun showBusPickerDialog() {
        ...
        items.addAll(RouteData.weekdayRoutes)
        items.add("Saturday Routes")
        items.addAll(RouteData.saturdayRoutes)

        val labels = items.map {
            if (it is String) {
                "=== $it ==="
            } else {
                val route = it as com.upsi.smartbus.model.Route
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setItems(labels) { _, which ->
                ...
                val selectedRoute = selected as com.upsi.smartbus.model.Route
